...
contract DividendPayingEUBIToken is IERC20, IERC20Metadata, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
	...
	function withdrawDividendTo(address addr) external {
		...
		if (_withdrawableDividend > 0) {
			withdrawnDividends[msg.sender] = withdrawnDividends[msg.sender].add(_withdrawableDividend);
			IERC20 just = IERC20(0x834295921A488D9d42b4b3021ED1a3C39fB0f03e);
			require(just.transfer(addr, _withdrawableDividend), "EUBIng2: can't transfer JUST Stablecoin");
			emit DividendWithdrawn(msg.sender, _withdrawableDividend);
		}
	}
	/// @notice Withdraws the ether distributed to the sender.
	...
	function withdrawDividendFor(address addr) external {
		require(canRecieveDividends(addr), "EUBIng2: dividends disabled");
		uint256 _withdrawableDividend = magnifiedDividendPerShare.mul(_balances[addr]).add(magnifiedDividendCorrections[addr]).div(magnitude).sub(withdrawnDividends[addr]);
		if (_withdrawableDividend > 0) {
